...
#########################################
#use this script from Yunjiang Qiu to generate 21bp sequences,and 11th is reference allele or alternative allele of the SNP.
import sys
import argparse
import pysam
    ...
    with open (args.snp, "r") as f:
        for line in f:
            ...
            if len(ref) > 1 or len(alt) > 1:
                continue
            oligo_seq = fastafile.fetch(chrom, int(pos)-11, int(pos)+10).upper()
            if oligo_seq[10] != ref and oligo_seq[10] != alt:
                sys.stderr.write("{0} is not matched to the genome.\n".format(line.rstrip()))
                continue
            f1.write(">{0}_{1}_{2}_{3}\n".format(chrom, pos, ref, alt))
            f1.write("{0}\n".format((oligo_seq[0:10] + ref + oligo_seq[11:])))
            f2.write(">{0}_{1}_{2}_{3}\n".format(chrom, pos, ref, alt))
            f2.write("{0}\n".format((oligo_seq[0:10] + alt + oligo_seq[11:])))
