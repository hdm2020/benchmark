...
unzip master.zip 
mv deltaSVM-master/* ./
rm -rf deltaSVM-master/

#download resources files(including reference genome and weights)
mkdir resources
cd resources
wget -c -r -np -nd -k -L -p http://renlab.sdsc.edu/yunjiang/deltaSVM/resources/
rm index*
cd ../
#see https://github.com/ren-lab/deltaSVM/README.md
#get one directory: resources(including files:hg19.fa  hg19.fa.fai  models.weights.txt  threhsolds.obs.tsv  threhsolds.pbs.tsv)

#1.2 all gkm-SVM models
mkdir deltasvm_weights
cd deltasvm_weights
wget -c -r -np -nd -k -L -p http://renlab.sdsc.edu/yunjiang/deltaSVM/deltasvm_weights/
rm index*
cd ../
#see https://github.com/ren-lab/deltaSVM/README.md
...
#all 533 TFs 
ls deltasvm_weights/ > 533gkmsvm_models.txt
...
mv besttfmodel.roc.prc.txt $datadir/deltaSVM_HT-SELEX.alltf.bestmodel.roc.prc.txt
