...
#########################################
##use this script from Yunjiang Qiu to generate 101bp sequences,and 51th is reference allele or alternative allele of the SNP.
import sys
import argparse
import pysam
    ...
    with open (args.snp, "r") as f:
        for line in f:
            ...
            if len(ref) > 1 or len(alt) > 1:
                continue
            oligo_seq = fastafile.fetch(chrom, int(pos)-51, int(pos)+50).upper()
            if oligo_seq[50] != ref and oligo_seq[50] != alt:
                sys.stderr.write("{0} is not matched to the genome.\n".format(line.rstrip()))
                continue
            f1.write(">{0}_{1}_{2}_{3}\n".format(chrom, pos, ref, alt))
            f1.write("{0}\n".format((oligo_seq[0:50] + ref + oligo_seq[51:])))
            f2.write(">{0}_{1}_{2}_{3}\n".format(chrom, pos, ref, alt))
            f2.write("{0}\n".format((oligo_seq[0:50] + alt + oligo_seq[51:])))
