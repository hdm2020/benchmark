...
outfile<-'./testdata/deltaSVM_ChIP-seq.merged.expe.pred.results.txt'

#read prediction result
model_tf<-read.csv(modeltf,stringsAsFactors=F)
result<-data.frame()
for (i in 1:nrow(model_tf)){
  df<-read.table(paste0('testdata/results/',model_tf[i,'weight']),header=F,stringsAsFactors=F,sep='\t')
