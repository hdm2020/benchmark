#script for predicting snp's effect on TF binding using DeepBind_HT-SELEX
...
#2 modify names of TFs DeepBind_HT-SELEX can predict as their standard HGNC SYMBOL
awk -F '\t' '{print $1}' ~/.kipoi/models/DeepBind/models.tsv|grep 'Homo_sapiens/TF/'|grep 'SELEX' > hg.selex.378tf.models.txt
Rscript TFrename.R
...
Rscript --vanilla auroc_auprc.R -e ../../snpdata/testdata/evaldata -f $datadir/DeepBind_HT-SELEX.merged.expe.pred.results.txt -m $datadir/evaldata_interselex377tf.csv -d delta_alt_ref -o $datadir/DeepBind_HT-SELEX.tf.roc.prc.txt
