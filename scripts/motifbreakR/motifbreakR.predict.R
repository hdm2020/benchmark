...
selectpwm<-function(pwmdb){
  if(pwmdb=='jaspar'){
    ...
    motiflist$id<-apply(data.frame(motiflist$model_name),1,id)
    xx<-subset(MotifDb,dataSource=='jaspar2022' & organism=='Hsapiens')#691
    motiflibrary<-query(xx,andStrings=c('Hsapiens','jaspar2022'),orStrings=motiflist$id)
    return(motiflibrary)
    }
