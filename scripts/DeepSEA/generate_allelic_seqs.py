    ...
    with open (args.snp, "r") as f:
        for line in f:
            ...
            if len(ref) > 1 or len(alt) > 1:
                continue
            oligo_seq = fastafile.fetch(chrom, int(pos)-500, int(pos)+500).upper()
            if oligo_seq[499] != ref and oligo_seq[499] != alt:
                sys.stderr.write("{0} is not matched to the genome.\n".format(line.rstrip()))
                continue
            f1.write(">{0}_{1}_{2}_{3}\n".format(chrom, pos, ref, alt))
            f1.write("{0}\n".format((oligo_seq[0:499] + ref + oligo_seq[500:])))
            f2.write(">{0}_{1}_{2}_{3}\n".format(chrom, pos, ref, alt))
            f2.write("{0}\n".format((oligo_seq[0:499] + alt + oligo_seq[500:])))
